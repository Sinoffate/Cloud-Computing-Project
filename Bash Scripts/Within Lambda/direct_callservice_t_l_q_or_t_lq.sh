...
# direct call T -> L-Q and T -> LQ functions are stored on handler_T_hww lambda function, 
# when we calling different python files, we just change the Runtime Setting handler info.

# Lambda Function name and AWS region
lambda_function_name="handler_T_hww"
region="us-east-2"
...
output_file="$output_dir/results_handler_T_hww.txt"
